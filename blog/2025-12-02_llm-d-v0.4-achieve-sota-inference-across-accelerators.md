...
In our [user guide benchmarking](/docs/dev/well-lit-paths/tiered-prefix-cache), enabling CPU offloading when KV cache working set exceeded the available HBM capacity yielded a **25% reduction in mean Time To First Token (TTFT)** and a **21% increase in total throughput**.
...
[*Table: High Performance*](/docs/dev/well-lit-paths/tiered-prefix-cache) *compares the performance of the baseline vLLM with the vLLM using the CPU offloading connector when the KVCache size is larger than the available HBM.)*
